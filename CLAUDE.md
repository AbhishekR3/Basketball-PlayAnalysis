...
1. **Passing_Simulation.py** or **RandomMovement_Simulation.py** — Pygame-based generators that render synthetic basketball plays to MP4 in `$VIDEO_DIR`. `Passing_Simulation` produces labeled passing plays (positive class); `RandomMovement_Simulation` produces unstructured movement (negative class). Both draw randomness from a module-level seeded `_rng = np.random.default_rng(...)` (seeded from the `SIM_SEED` env var, default 42; `seed_rng(seed)` reseeds for a bit-for-bit reproducible run) and a `cryptographic_normal` helper that Box-Muller-transforms `_rng` draws.
...
- **Randomness.** Simulations use a module-level seeded `_rng = np.random.default_rng(...)` (P0-H) — seeded from `SIM_SEED` (default 42), reseeded via `seed_rng(seed)` for reproducible runs. The older `secrets.SystemRandom()` draws were replaced by this; some `cryptographic_normal` docstrings/log strings still say "secrets" but the actual draws now come from `_rng`. The Neural Network separately sets `SEED = 42` for `torch` and `numpy`. Don't mix the sim `_rng` and the NN seed.
